...
class RealMultiscaleSRDataset(Dataset):
    def __init__(self, dataroot, scale:list, preload=True, patch_size=64, dataset_size=-1, batch_size=1, mode="Train"):
        self.dataroot = dataroot

        if mode not in ("train", "validate"):
            raise ValueError(f"Invalid mode: {self.mode}. mode must be either 'train' or 'validate'.")
        self.mode = mode

        self.base_paths = [os.path.join(self.dataroot, str(i)) for i in [2, 3, 4]]
        self.preload = preload
        self.patch_size = patch_size
        ...
        if preload:
            ...
            if not dist.is_available() or not dist.is_initialized() or dist.get_rank() == 0:
                print(f"Preloading {self.mode} dataset...")
            ...
            if not dist.is_available() or not dist.is_initialized() or dist.get_rank() == 0:
                print(f"Preloading {self.mode} dataset done")
    ...
    def __getitem__(self, idx):
        ...
        if self.patch_size is not None:
            transforms.append(crop(self.patch_size))

        if self.mode == "train":
            img_index = random.randint(1, self.real_sizes[0])
            transforms.append(augmentation())
        else:  # validate 모드에서는 순차적으로 이미지를 불러옴.
            img_index = idx + 1

        HR_img_path = os.path.join(self.base_paths[0], 'Canon_{:03d}_HR.png'.format(img_index))
        ...
        if self.preload:
            HR, LR2, LR3, LR4 = self.imgs[HR_img_path], self.imgs[LR2_img_path], self.imgs[LR3_img_path], self.imgs[LR4_img_path]
        else:
            HR = np.array(Image.open(HR_img_path).convert('RGB'))
        ...
        if transforms:
            for tr in transforms:
                HR, LR2, LR3, LR4 = tr(HR, LR2, LR3, LR4)
        else:
            HR = shave_on_four(HR)
            LR2 = shave_on_four(LR2)
            LR3 = shave_on_four(LR3)
        ...
        img_item = {}
        img_item['HR'] = HR.transpose(2, 0, 1).astype(np.float32) / 127.5 - 1
        img_item['LR2'] = LR2.transpose(2, 0, 1).astype(np.float32) / 127.5 - 1
        img_item['LR3'] = LR3.transpose(2, 0, 1).astype(np.float32) / 127.5 - 1
        img_item['LR4'] = LR4.transpose(2, 0, 1).astype(np.float32) / 127.5 - 1
        img_item['idx'] = img_index
